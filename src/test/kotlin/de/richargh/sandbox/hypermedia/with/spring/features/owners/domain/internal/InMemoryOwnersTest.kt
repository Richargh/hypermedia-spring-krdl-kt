...
class InMemoryOwnersTest {

    val subject = InMemoryOwners()
    ...
    inner class `With 10 items` {
        ...
        fun `search with offset 8 should return last two items`() {
            ...
            // THEN
            assertThat(result.items.map { it.id }).containsExactly(OwnerId("9"), OwnerId("10"))
        }
        ...
        fun `search with limit 2 should return first two items`() {
            ...
            // THEN
            assertThat(result.items.map { it.id }).containsExactly(OwnerId("1"), OwnerId("2"))
        }
        ...
        fun `search with limit 1 and offset 4 should have only the middle item`() {
            ...
            // THEN
            assertThat(result.items.map { it.id }).containsExactly(OwnerId("5"))
        }
    ...
    private fun anOwner(name: String = "Alex") = Owner(OwnerId((subject.count() + 1).toString()), name)
}